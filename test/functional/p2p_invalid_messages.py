...
class InvalidMessagesTest(BitcoinTestFramework):
    ...
    def test_large_inv(self):
        ...
        with self.nodes[0].assert_debug_log(['Misbehaving', '(0 -> 20) reason: oversized-inv']):
            msg = msg_inv([CInv(MSG_TX, 1)] * 50001)
            conn.send_and_ping(msg)
        with self.nodes[0].assert_debug_log(['Misbehaving', '(20 -> 40) reason: too-many-inv']):
            msg = msg_getdata([CInv(MSG_TX, 1)] * 50001)
            conn.send_and_ping(msg)
        with self.nodes[0].assert_debug_log(['Misbehaving', '(40 -> 60) reason: too-many-headers']):
